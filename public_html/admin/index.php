...
// Build apps list menu
  ...
  // Start page url example:http://localhost/litecart/public_html/admin/?app=catalog&doc=csv
    if (empty($_GET['app'])) {// if paramater is empty。
  ...
  //==============================================================================================================
      ...
      if (empty(user::$data['permissions'])
          || (!empty(user::$data['permissions'][$_GET['app']]['status'])
              && in_array($_GET['doc'], user::$data['permissions'][$_GET['app']]['docs']))) {

        require vmod::check(FS_DIR_HTTP_ROOT . WS_DIR_ADMIN . $_GET['app'].'.app/config.inc.php');

        if (empty($app_config['theme']['icon'])
            && !empty($app_config['icon'])) $app_config['theme']['icon'] = $app_config['icon']; // Backwards compatibility
