<?php
    /**
     * public_html/includes/templates/default.catalog/views/box_site_menu.inc.php
     */
  ...
  // Categories
       ...
       */
      // 一级分类
    $categories_query = functions::catalog_categories_query(0, 'tree');
    while ($category = database::fetch($categories_query)) {// 一级分类
      ...
      // 二级分类
      $subcategories_query = functions::catalog_categories_query($category['id'],'tree');
      while ($subcategory = database::fetch($subcategories_query)) {
        $box_site_menu->snippets['categories'][$category['id']]['subitems'][$subcategory['id']] = array(
          ...
          'priority' => $subcategory['priority'],
            ...
            // 三级分类
//          $three_subcategories_query = functions::catalog_categories_query($subcategory['id'],'tree',30);
