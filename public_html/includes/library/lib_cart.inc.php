...
//          $item_key = $item['key'];
                self::add_product($product_id, $options, $quantity);
            }
        ...
        public static function clear()
        {
            ...
            self::reset();
            $sql = "delete from " . DB_TABLE_CART_ITEMS . " where cart_uid = '" . database::input(self::$data['uid']) . "';";
            database::query($sql);
        }
        ...
        public static function load()
        {
            ...
            if (!empty(customer::$data['id'])) {
                $sql = "update " . DB_TABLE_CART_ITEMS . " set cart_uid = '" . database::input(self::$data['uid']) . "',
                customer_id = " . (int)customer::$data['id'] . " where cart_uid = '" . database::input(self::$data['uid']) . "'
                or customer_id = " . (int)customer::$data['id'] . ";";
                database::query($sql);
            }
            $sql = "select * from " . DB_TABLE_CART_ITEMS . " where cart_uid = '" . database::input(self::$data['uid']) . "';";
            $cart_items_query = database::query($sql);

            while ($item = database::fetch($cart_items_query)) {
                // Remove duplicate cart item if present
                if (!empty(self::$items[$item['key']])) {
                    $sql ="delete from " . DB_TABLE_CART_ITEMS . " where cart_uid = '" . database::input(self::$data['uid']) . "'
                    and id = " . (int)$item['id'] . " limit 1;";
                    database::query($sql);
                }
                ...
                self::add_product($product_id, $options, $quantity, true, $item_key);
        ...
        public static function add_product($product_id, $options, $quantity = 1, $force = false, $item_key = null)
        {
            ...
            try {
                ...
                foreach ($product->options as $option) {
                    // Check group
                    $group_options = reference::option_group($option['id']); // new ctrl_option_group object.
                    $possible_groups = array_filter(array_unique($group_options->name));// 拿到对应的组名集合
                    ...
                    switch ($option['function']) {
                        ...
                        case 'checkbox':
                            ...
                            foreach ($option['values'] as $value) {
                                $possible_values = array_filter(array_unique($group_options->values[$value['id']]['name']));
            ...
            if (!empty(self::$items[$item_key]['id'])) {
                $sql = "update " . DB_TABLE_CART_ITEMS . " set quantity = " . (float)self::$items[$item_key]['quantity'] . ",
                        date_updated = '" . date('Y-m-d H:i:s') . "'
                        where cart_uid = '" . database::input(self::$data['uid']) . "'
                        and `key` = '" . database::input($item_key) . "' limit 1;";
                database::query($sql);
            } else {
                if (!$force) {
                    $sql = "insert into " . DB_TABLE_CART_ITEMS . " (customer_id, cart_uid, `key`, product_id, options, quantity, date_updated, date_created)
                    values (" . (int)customer::$data['id'] . ", '" . database::input(self::$data['uid']) . "', '" .
                        database::input($item_key) . "', " . (int)$item['product_id'] . ", '" .
                        database::input(serialize($item['options'])) . "', " . (float)$item['quantity'] . ", '" .
