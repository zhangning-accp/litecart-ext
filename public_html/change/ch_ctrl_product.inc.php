  ...
  class ctrl_product
  {
    ...
    public function load($product_id)
    {
      ...
      $products_query = database::query(
        "select * from " . DB_TABLE_PRODUCTS . " where id = '" . (int)$product_id . "' limit 1;"
      );
    ...
    public function save()
    {
       ...
       *  开始更新相关表
       */
      // 1 Update product_to_categories
      $this->update_product_to_categories();
      // 2 Update product_info
      $this->update_product_info();
      // 3 Update Price
      $this->update_prices_table();
      // 4 Update
    ...
    //Update campaigns
    private function update_campaigns()
    {
      // Delete campaigns
      database::query(
        "delete from " . DB_TABLE_PRODUCTS_CAMPAIGNS . "
    ...
    //Update prices table
    private function update_prices_table()
    {
// Price select->insert->update
      foreach (array_keys(currency::$currencies) as $currency_code) {
...
// Update product_info. find->insert->update
    ...
    private function update_product_to_categories()
    {
